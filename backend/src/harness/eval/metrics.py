...
async def compute_metric(
    config: MetricConfig, agent_response: AgentResponse,
    expected_output: str | None, expected_tools: list[ToolCall] | None,
    ...
    if config.backend == MetricBackend.ADK:
        ...
        score = await _compute_deepeval_metric(config, agent_response, expected_output, reference_contexts)
    else:
        raise ValueError(f"Unknown backend: {config.backend}")
    return MetricResult(
        name=config.name, score=score, threshold=config.threshold,
        details={"backend": config.backend.value}, backend=config.backend.value,
    )
...
async def _compute_deepeval_metric(
    config: MetricConfig, agent_response: AgentResponse,
    expected_output: str | None, reference_contexts: list[str] | None,
) -> float:
    ...
    test_case = LLMTestCase(
        input=expected_output or "", actual_output=agent_response.output,
        expected_output=expected_output, retrieval_context=reference_contexts or [],
        context=reference_contexts or [],
    )
    metric_map = {
        "faithfulness": FaithfulnessMetric, "contextual_relevancy": ContextualRelevancyMetric,
        "hallucination": HallucinationMetric, "answer_relevancy": AnswerRelevancyMetric,
    }
